...
**⛔ 正文不写开头 H1（硬性约定）**：公众号推文标题由发布环节 `--title` 单独控制，正文开头**不要**写 `# 标题`（避免推文标题与正文大标题重复）。即使风格文件模板示例含 `# 标题`，生成初稿时也要跳过——`article-raw.md` 第一行直接是正文段落或 `##` 小节标题。若正文中确需突出主题，用 `##`（H2）而非 `#`（H1）。

**用户干预点（交互式确认）**：初稿产出后，用 `AskUserQuestion` 交互式询问（选项示例）：
1. `确认继续` — 用户觉得满意，不需要修改，直接进入 Step 3
2. `我要修改` — 用户自行删改 `article-raw.md`（或告知修改要点由 AI 代改），删改完成后再次询问，确认满意后进入 Step 3

用户未回复前不得进入 Step 3。若用户选择修改，修改完成后必须再次交互确认，确认满意后才能继续。
...
>
> **⛔ 格式化后检查开头 H1**：`baoyu-format-markdown` 可能补出/保留开头 `# 标题`（H1）。格式化完成后，检查输出文件第一行是否为 `# ` 开头——若是，删除该行（公众号标题由发布 `--title` 控制，正文不应重复大标题）。此检查在去 AI 味（Step 4）前完成，`article.md` 最终不得含开头 H1。
...
- **标题处理（硬性）**：easy 系列默认保留正文首个 `# 标题`；**发布前必须确保正文无 H1**（公众号标题由发布环节 `--title` 控制，正文不应重复大标题）。若 `article.md` 开头仍有 `# 标题`（Step 3 兜底检查漏网时），发布前删除该行并重新转 HTML
