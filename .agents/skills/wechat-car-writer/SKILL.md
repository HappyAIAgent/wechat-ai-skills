...
> **各品牌最新车型的官网车型页 URL 清单**：见本 skill `references/汽车官网车型页地址库.md`（59 个品牌，2026-08 实测验证；含已剔除项与 SPA/子站/域名等维护提示）。`car-news-images.ts` 的 `OFFICIAL_SITES`（品牌级）与 `OFFICIAL_MODEL_SITES`（车型级，419 条）映射据此维护，新增/失效车型先查该清单再改映射。
